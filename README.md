...
## Integração com API
- Foi colocado json static fora do db.json usado com json-server, porque quando for subir essa aplicação de demonstração não terá a Api fake em paralelo, precisando estar no projeto. 
- Depois de integrado com API correta pode-se apagar a pasta `src/assets/mocks`.
- Para usar o mock static, basta colocar um json na pasta citada acima e chamar como abaixo num service que extends BaseService.

```ts
// COM MOCK
public getListarProcessos = (processo: any): Observable<any> => {
    return this.get('/grid', null, TiposApisEnum.STATIC);
};
// COM API OK
public getListarProcessos = (processo: any): Observable<any> => {
